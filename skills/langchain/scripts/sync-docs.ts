#!/usr/bin/env bun
/**
 * Sync LangChain documentation from the official langchain-ai/docs repository.
 *
 * Usage:
 *   bun run sync          # Sync all docs
 *   bun run sync:dry      # Preview without writing
 *
 * Clones the docs repo and copies only the LangChain-specific documentation
 * from src/oss/langchain/, excluding LangGraph and LangSmith.
 */

import { parseArgs } from "util";
import { mkdir, writeFile, readFile, rm, readdir, cp } from "fs/promises";
import { join } from "path";
import { execSync } from "child_process";
import { existsSync } from "fs";

const REPO_URL = "https://github.com/langchain-ai/docs.git";
const SOURCE_PATH = "src/oss/langchain";
const RESOURCES_DIR = join(import.meta.dir, "..", "resources");
const TEMP_DIR = "/tmp/langchain-docs-sync";
...
async function main() {
  ...
  console.log(`\nSyncing LangChain docs${dryRun ? " (dry run)" : ""}...\n`);
