#!/usr/bin/env bun
/**
 * Sync OpenCode documentation from the official sst/opencode repository.
 *
 * Usage:
 *   bun run sync          # Sync all docs
 *   bun run sync:dry      # Preview without writing
 *
 * Clones the repo and copies documentation from packages/web/src/content/docs/.
 */
...
const SOURCE_PATH = "packages/web/src/content/docs";
const RESOURCES_DIR = join(import.meta.dir, "..", "resources");
const TEMP_DIR = "/tmp/opencode-docs-sync";
...
async function cloneRepo(): Promise<void> {
  console.log("Cloning sst/opencode repository...");
...
async function main() {
  ...
  console.log(`\nSyncing OpenCode docs${dryRun ? " (dry run)" : ""}...\n`);
